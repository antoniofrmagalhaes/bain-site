...
export type ServiceType =
  ...
  | 'Consultoria em Sistemas Legado'
  | 'Automacao de Processos'
  | 'Criacao de Websites e Landing Pages';
...
export type FormValues = {
  answer: string; // A resposta do usuário
};
...
export const QuestionsContext = createContext<IQuestionsContextProps>({} as IQuestionsContextProps);
...
export function useProvideQuestions(): IQuestionsContextProps {
  ...
  const [data, setData] = useState<{ questionary: { question: string; answer: string }[] }>({ questionary: [] });
  const [userName, setUserName] = useState('Usuário');

  const { register, handleSubmit, watch, trigger, setValue, resetField, setFocus } = useForm<FormValues>({
    defaultValues: {
      answer: '',
    },
  });
  ...
  const initialQuestions: Question[] = [
    ...
    {
      question: `Olá ${userName || 'Usuário'}, Qual tipo de serviço você gostaria de contratar?`,
      inputType: 'choice',
      choices: [
  ...
  const serviceQuestions: Record<ServiceType, Question[]> = {
    'Software House': [
      {
        ...
        placeholder: 'Ex.: Um sistema de gerenciamento de pedidos para restaurantes.',
        inputType: 'input',
      },
      {
        question: 'Quais são as principais funcionalidades que você precisa no software?',
        placeholder: 'Ex.: Controle de usuários, relatórios, integração com APIs.',
        inputType: 'input',
      },
      {
        question:
      ...
      {
        question: 'Quando você gostaria de ver a primeira versão do seu software pronta?',
        inputType: 'choice',
        choices: [
      ...
      {
        question: 'Qual seria o orçamento ideal para você investir nesse software?',
        inputType: 'choice',
        choices: [
    ...
    'Integracoes de Sistemas': [
      ...
      {
        question: 'Existem especificações ou APIs disponíveis para os sistemas que você quer integrar?',
      ...
      {
        question: 'Qual seria o orçamento ideal para você investir nessas integrações?',
        inputType: 'choice',
        choices: [
          { key: 1, choice: 'Até R$ 5.000' },
          { key: 2, choice: 'R$ 5.000 - R$ 25.000' },
  ...
  const questions = [...initialQuestions, ...(selectedService ? serviceQuestions[selectedService] : [])];
    ...
    if (questionIndex === 1) {
      ...
      setData(prevData => {
        ...
        if (updatedQuestionary[questionIndex]) {
          updatedQuestionary[questionIndex] = { question: questions[questionIndex]?.question, answer: selectedChoice };
        } else {
          updatedQuestionary.push({ question: questions[questionIndex]?.question, answer: selectedChoice });
        }
  ...
  const handleNext = async () => {
    ...
    if (isValid) {
      ...
      setData(prevData => {
        const updatedQuestionary = [...prevData.questionary];
        if (updatedQuestionary[questionIndex]) {
          updatedQuestionary[questionIndex] = { question: questions[questionIndex]?.question, answer: watch('answer') };
        } else {
          updatedQuestionary.push({ question: questions[questionIndex]?.question, answer: watch('answer') });
        }
        return { questionary: updatedQuestionary };
      });
